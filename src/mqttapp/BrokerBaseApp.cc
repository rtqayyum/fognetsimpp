...
Define_Module(BrokerBaseApp);

simsignal_t BrokerBaseApp::pkSignal = registerSignal("pk");

void BrokerBaseApp::initialize(int stage)
{
...
void BrokerBaseApp::handleMessageWhenUp(cMessage *msg)
{
    ...
    if (msg->isSelfMessage()) {
        ...
        if (dynamic_cast<MqttMsgConnect* >(msg)) {
            ...
            FognetMsgTaskAck *tskAck = check_and_cast<FognetMsgTaskAck *>(msg);

            EV<<"task status"<<tskAck->getStatus();
...
void BrokerBaseApp::sendPubAck(MqttMsgPublish *mqttPkt, L3Address ip, int prt, bool status){
...
void BrokerBaseApp::finish()
{
    ApplicationBase::finish();
}

bool BrokerBaseApp::handleNodeStart(IDoneCallback *doneCallback)
{
...
void BrokerBaseApp::handleNodeCrash()
{
}
Client* BrokerBaseApp::findClient(const char* clientID){
    Client* c=new Client();
    for(int i=0; i<10; i++){
...
void BrokerBaseApp::publishAll(cMessage *msg){
...
std::string BrokerBaseApp::RandomString(unsigned int len)
